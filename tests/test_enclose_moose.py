...
def create_level(client, token, **kwargs):
    ...
    return client.post("/enclose-moose/admin", json=default_data | kwargs, headers=auth_headers(token))


def patch_level(client, token, level_id, **kwargs):
    return client.patch(f"/enclose-moose/admin/{level_id}", json=kwargs, headers=auth_headers(token))


def delete_level(client, token, level_id):
    return client.delete(f"/enclose-moose/admin/{level_id}", headers=auth_headers(token))


def admin_get_level(client, token, level_id):
    return client.get(f"/enclose-moose/admin/{level_id}", headers=auth_headers(token))


def get_level(client, token, level_id):
    return client.get(f"/enclose-moose/{level_id}", headers=auth_headers(token))


def admin_get_all_levels(client, token):
    return client.get(f"/enclose-moose/admin", headers=auth_headers(token))


def get_all_levels(client, token):
    return client.get(f"/enclose-moose", headers=auth_headers(token))
...
def submit_solution(client, token, level_id, player_solution, secret_header="happy_secret_key"):
    ...
    return client.post(f"/enclose-moose/{level_id}/submit", json=body, headers=headers)


def get_submission(client, token, level_id):
    return client.get(f"/enclose-moose/{level_id}/submit", headers=auth_headers(token))
...
def test_get_levels(client, member_token, admin_token):
...
def test_submission(client, member_token, admin_token):
    res_non_existant = submit_solution(client, member_token, "released_test", player_solution=[3, 5, 7])
    assert res_non_existant.status_code == 404

    res_get_non_existant = get_submission(client, member_token, "released_test")
    assert res_get_non_existant.status_code == 404
...
def test_non_member_cannot_access_member_routes(client, non_member_token, admin_token):
    ...
    res_get_level_non_member = get_level(client, non_member_token, "released_test")
    assert res_get_level_non_member.status_code == 403

    res_get_all_levels_non_member = get_all_levels(client, non_member_token)
    assert res_get_all_levels_non_member.status_code == 403

    res_submit_non_member = submit_solution(client, non_member_token, "released_test", player_solution=[3, 5, 7])
    assert res_submit_non_member.status_code == 403

    res_get_submission_non_member = submit_solution(
        client, non_member_token, "released_test", player_solution=[3, 5, 7]
    )
    assert res_get_submission_non_member.status_code == 403
