...
def test_admin_can_select_any_council(client, admin_token, admin_council_id):
...
def test_personal_booking_creation_user(client, member_token, member_council_id):
    ...
    data = resp.json()
    assert data["personal"] is True
    assert data["council_id"] == member_council_id
...
def test_personal_booking_council_required(client, member_token, member_council_id):
    start = stockholm_dt(2030, 3, 9, 10)
    end = stockholm_dt(2030, 3, 9, 12)

    # Personal booking still requires valid council
    resp = create_booking(
        client, member_token, start, end, "personal without council", personal=True, council_id=member_council_id
    )
