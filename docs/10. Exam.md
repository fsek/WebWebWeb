...
`Student.card_pin` and `Book.user_id` are considered secret fields which must be filtered out of any response our server gives the client.
...
- `/users`: Create a User with appropriate data.
...
- `/borrow-book`: Assign a book to a user, both with id's given in the body.
