...
@car_router.get("/{booking_id}", response_model=CarRead, dependencies=[Permission.require("view","Car"), Permission.member()])
...
def remove_booking(booking_id:int,current_user:Annotated[User_DB, Permission.member()],manage_permission:Annotated[bool, Permission.check("manage","Car")], db: DB_dependency):
...
def update_booking(booking_id:int,data:CarUpdate,current_user:Annotated[User_DB, Permission.member()],manage_permission:Annotated[bool,Permission.check("manage", "Car")],db:DB_dependency):
    car_booking = db.query(CarBooking_DB).filter(CarBooking_DB.booking_id==booking_id).first()
    if car_booking is None:
