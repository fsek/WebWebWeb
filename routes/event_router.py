...
from api_schemas.event_schemas import AddEventTag, EventCreate, EventPriorityRead, EventRead, EventUpdate
...
@event_router.get("/priorities/{event_id}", response_model=list[EventPriorityRead])
def get_event_priorities(event_id: int, db: DB_dependency):

    priorities = db.query(Priority_DB).join(Event_DB).filter(Event_DB.id == event_id).all()
