...
def create_course(data: CourseCreate, db: DB_dependency):
    ...
    existing_title = db.query(Course_DB).filter_by(title_urlized=normalized_title).one_or_none()
...
def update_course(course_id: int, data: CourseUpdate, db: DB_dependency):
    ...
    existing_title = (
        db.query(Course_DB)
        .filter(Course_DB.course_id != course_id)
        .filter_by(title_urlized=normalized_title)
        .one_or_none()
    )
    if existing_title is not None:
        raise HTTPException(status.HTTP_409_CONFLICT, detail="Course title already exists")
